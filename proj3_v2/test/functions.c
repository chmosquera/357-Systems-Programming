...
struct LLRule * BuildRules(FILE * file){ 
  ...
  /*      ProcessLine(line, list_acts, list_deps, rule, node_rule, list_rules);*/
        if (line[0] == '\0') {} 
        else if (line[0] == '\t') {
            ...
            if ((target = strtok(line, " ")) == NULL) {
               printf( "\nError: Trying to build list of rules. Data expected\n");
            }
...
/*
void ProcessLine(char * line, struct LLDepAct * list_acts, struct LLDepAct * list_deps, struct Rule * rule, struct NodeRule * node_rule, struct LLRule * list_rules) {
    char * target, * sep; 
    struct NodeDepAct *node_act;
        if (line[0] == '\0') {} 
        else if (line[0] == '\t') {
...
void ExecuteActions(struct LLDepAct *list_acts) {
    ...
    act = list_acts->head;
/*    printf("Executing actions for: %s\n", act->depAct); */
    while (act != NULL) {
        fprintf(stdout, "%s\n", act->depAct);
